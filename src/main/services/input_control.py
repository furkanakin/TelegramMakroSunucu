...
# Key Map for common non-char keys
VK_MAP = {
    "Enter": 0x0D,
    "Backspace": 0x08,
    "Tab": 0x09,
...
def mouse_scroll(delta):
    ...
    # We negate it because JS deltaY is positive for down
...
def key_press(key):
    ...
    if key in VK_MAP:
        ...
        # For single characters, get the VK code
        # user32.VkKeyScanW returns the VK code in the low-order byte
        res = user32.VkKeyScanW(ord(key))
        if res != -1:
            vk = res & 0xFF
            # If shift is required (bit 8), we'd need to press shift too
            # For now, let's just do the simple press
